...
def bot_interaction(bot1, bot2, conversation_topics=None):
    ...
    if conversation_topics is None:
        conversation_topics = get_valid_input("Enter conversation topics separated by commas: ", [])
        conversation_topics = [topic.strip() for topic in conversation_topics.split(',')]
...
def suggest_date_activities(bot1, bot2, date_activities):
    ...
    for activity in selected_activities:
        if activity in date_activities:
            ...
            print("Error: Activity not found in the list of date activities.")
...
def express_feelings(bot1, bot2):
    ...
    print('\n'.join(additional_responses_bot1))
    print('\n'.join(additional_responses_bot2))

def closing_statement(bot1, bot2):
    print(f"Thank you, {bot1} and {bot2}, for a wonderful date! Let's plan our next adventure soon.")
