...
class UserGenerator:
    ...
    def generate_user(self):
        ...
        traits = random.sample(self.personalities, 3)
    ...
    def compute_occurrence_phrase(self, name):
        occurrence = 's' if len(self.user_history[name]) > 1 else ''
        return occurrence
    ...
    def get_user_profile(self, name):
        if name in self.user_history:
            ...
            return f"{name} has a {personality} personality with Traits: {trait_string}. They have been generated {len(self.user_history[name])} time{occurrence}."
        else:
